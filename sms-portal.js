...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Actually call the initialization
...
async function initializeApplication() {
    ...
    try {
        // Make sure smsAPI is assigned from window
        if (!smsAPI && window.smsAPI) {
            smsAPI = window.smsAPI;
            console.log('✅ SMS API assigned from window.smsAPI');
        }
        
        // 1. Setup UI elements first
        initializeSelectElements();
        console.log('✅ Select elements initialized');
        ...
        // 5. Load sites and staff data (don't wait)
        loadSiteData().catch(e => console.warn('Sites load error:', e));
        loadStaffData().catch(e => console.warn('Staff load error:', e));
        
        // 6. Update SMS balance
        updateSMSBalance().catch(e => console.warn('Balance update error:', e));
...
function initializeSelectElements() {
    // Initialize Select2 for enhanced dropdowns (NOT customer - keep it simple)
    $('.select2').select2({
...
async function loadStaffData() {
    try {
        ...
        staffData = staffRecords.map(staff => {
            let phoneNumber = staff.cellphone_number || staff.mobile || staff.contact_number || null;
            
            let staffName = staff.full_name || staff.name || 
                           (staff.first_name ? `${staff.first_name} ${staff.last_name || ''}` : 'Staff') ||
            ...
            return {
                ...
                site_id: staff.store_code || staff.site_id,
                site_name: staff.store_name || staff.site_name || 'Unknown Site',
                customer_id: staff.customer_id || 'MAKRO',
                customer_name: staff.customer_name || 'MAKRO',
                department_id: staff.sub_department || staff.department_id,
                position_id: staff.job_title_id || staff.position_id,
                position_title: staff.job_title || staff.position || 'Staff',
                active: staff.status === 'ACTIVE',
                email: staff.email_address || staff.email,
...
async function loadSiteData() {
    try {
        ...
        console.log('📋 Site query result:', { 
            rowCount: siteRecords?.length, 
            error: error?.message,
            firstThree: siteRecords?.slice(0, 3)
        });

        if (error) {
            console.error('❌ Sites query error:', error.message);
            siteData = [];
            ...
            console.log('✅ Loaded', siteData.length, 'sites');
        } else {
            console.warn('⚠️ No sites found');
            siteData = [];
        }
        ...
        console.error('❌ Site load error:', error.message);
        siteData = [];
        populateSiteDropdown();
    }
}
...
function populateSiteDropdown() {
    ...
    if (selectedCustomerId) {
        ...
        selectedCustomerName = customer ? customer.name : null;
    }
    
    // Filter sites by customer (case-insensitive)
    const sites = selectedCustomerName 
        ? siteData.filter(s => (s.customer || '').toUpperCase() === selectedCustomerName.toUpperCase())
        : siteData;
...
function populateCustomerDropdown() {
    ...
    if (!select) {
        console.warn('⚠️ customerSelect element not found');
        return;
    }
    ...
    console.log('✅ Dropdown populated with', customerData.length, 'customers');
}
...
async function sendSMS() {
    // Get SMS API from window if not in local scope
    if (!smsAPI && window.smsAPI) {
        smsAPI = window.smsAPI;
    }
    ...
    if (!smsAPI) {
        ...
        console.error('smsAPI is null. window.smsAPI:', window.smsAPI);
        return;
    }
